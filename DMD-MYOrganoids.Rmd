...
In this practice session I'm going to note down the steps to re-analyze an RNA-seq study. [Palmieri et al. (2025)](https://www.biorxiv.org/content/10.1101/2023.07.26.550063v4) generated an organoid-like structure from DMD muscle cells (derived from patient iPSCs) co-cultured with fibroblasts. This dataset contains **12 samples** of **3 conditions**: IsoCTR, Untreated (disease), and Treated (disease treated with high dose of AAV gene therapy). Each condition includes **4 biological replicates**. Raw data are available in the NCBI Bioproject database under accession PRJNA1208956.
...
<summary>**QC of raw sequencing reads**</summary>
...
Now we can [read the count table into R]{.underline} for differential expression analysis:

```{r read_count_table, echo=TRUE}
...
```{r echo=FALSE}
library(dplyr)
knitr::kable(head(count_tab), 
             caption = "Count table: genes as rows, samples as columns") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```

Also read the **metadata** file:

```{r read_metadata, echo=TRUE}
...
```{r top_gene_heatmap, echo=TRUE, fig.height=12, fig.width=8}
library("pheatmap")
...
```{r dist_matrix_heatmap, echo=TRUE, fig.height=8, fig.width=8}
...
<summary>To make volcano plot</summary>
